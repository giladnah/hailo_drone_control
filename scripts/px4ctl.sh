...
cmd_shell() {
    ...
    if docker compose ps dev 2>/dev/null | grep -q "running"; then
...
cmd_run() {
    ...
    if [ -z "$script" ]; then
        ...
        echo "Usage: $0 run <script>"
        echo "Example: $0 run examples/hover_rotate.py"
        exit 1
    fi
    ...
    if docker compose ps control 2>/dev/null | grep -q "running"; then
        docker compose exec control python3 "/workspace/$script"
    else
        log_warn "Control container not running. Starting it..."
        docker compose --profile control run --rm control python3 "/workspace/$script"
    fi
}
