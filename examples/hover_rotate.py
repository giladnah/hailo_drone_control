...
Usage:
    python3 hover_rotate.py [options]
...
Example:
    python3 hover_rotate.py --altitude 10 --rotation 720
"""
...
class HoverRotateDemo:
    ...
    async def connect(self, timeout: float = 30.0) -> bool:
        ...
        logger.info(f"Connecting to drone at {self.host}:{self.port}...")
    ...
    async def preflight_check(self) -> bool:
        ...
        async for attitude in self.drone.telemetry.attitude_euler():
            ...
            logger.info(f"  ✓ Initial heading: {self._initial_yaw:.1f}°")
            break
    ...
    async def rotate(self) -> bool:
        ...
        """
        logger.info(f"Starting {self.rotation}° rotation at {self.rotation_speed}°/s...")
        ...
        try:
            ...
            while abs(rotated) < abs(self.rotation):
                ...
                logger.info(f"  ⟳ Rotating: {abs(rotated):.1f}° / {abs(self.rotation)}° ({progress:.0f}%)")
            ...
            logger.info(f"  ✓ Rotation complete: {self.rotation}°")
            return True
    ...
    async def run(self) -> bool:
        ...
        logger.info(f"  Speed: {self.rotation_speed}°/s")
        logger.info("=" * 50)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="PX4 Hover & Rotate Demo - Takeoff, rotate 360°, and land"
    )
    parser.add_argument(
        "--altitude",
        type=float,
